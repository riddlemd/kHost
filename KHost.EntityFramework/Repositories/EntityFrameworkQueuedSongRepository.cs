...
namespace KHost.EntityFramework.Repositories
{
    internal class EntityFrameworkQueuedSongRepository : BaseEntityFrameworkRepository<QueuedSong>, IQueuedSongsRepository
    {
        ...
        public async Task<QueuedSong?> GetLastBySingerIdAsyncAsync(int queuedSingerId)
            => await Context.Set<QueuedSong>().Where(e => e.QueuedSingerId == queuedSingerId).OrderBy(e => e.Position).LastOrDefaultAsync();

        public async Task<IEnumerable<QueuedSong>> FindByQueuedSingerIdAsync(int id)
        {
        ...
        public async override Task CreateAsync(QueuedSong entity)
        {
            var lastQueuedSong = await GetLastBySingerIdAsyncAsync(entity.QueuedSingerId);
